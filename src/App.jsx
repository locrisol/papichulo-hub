...
// ── KPI Card ──────────────────────────────────────────────────────────────────
const KpiCard = ({ label, value, target, amount, status }) => {
...
// Login
const LoginScreen = () => (
  <div style={{ flex:1, display:"flex", alignItems:"center", justifyContent:"center", background:"linear-gradient(135deg, #182F24 0%, #2E5540 100%)" }}>
    <div style={{ width:380, background:T.card, borderRadius:20, padding:40, boxShadow:"0 24px 64px rgba(0,0,0,0.2)" }}>
      <div style={{ textAlign:"center", marginBottom:32 }}>
        <div style={{ display:"inline-flex", alignItems:"center", justifyContent:"center", width:56, height:56, background:T.accentL, borderRadius:16, marginBottom:16 }}>
          <span style={{ fontSize:28 }}>🌮</span>
        </div>
        <div style={{ fontFamily:"'Fraunces', serif", fontSize:24, fontWeight:700, color:T.text }}>Papi Chulo Hub</div>
        <div style={{ fontSize:13, color:T.muted, marginTop:4 }}>Business Management System</div>
      </div>
      <div style={{ marginBottom:16 }}>
        <div style={S.label}>Email address</div>
        <input style={S.input} defaultValue="leandro@papichulo.ie" />
      </div>
      <div style={{ marginBottom:24 }}>
        <div style={S.label}>Password</div>
        <input style={S.input} type="password" defaultValue="••••••••••" />
      </div>
      <button style={{ ...S.btn("primary"), width:"100%", justifyContent:"center", padding:"12px 18px", fontSize:15 }}>
        Sign in
      </button>
  ...
  if (showLogin) return (
    <div style={{ ...S.app, flexDirection:"column" }}>
      ...
      <div style={{ position:"fixed", bottom:20, left:"50%", transform:"translateX(-50%)" }}>
        <button onClick={() => setShowLogin(false)} style={{ ...S.btn("outline"), background:"#fff", fontSize:12 }}>
          ← Back to App
        </button>
      </div>
    </div>
      ...
      <div style={S.sidebar}>
        ...
        <div style={{ padding:"16px 20px", borderTop:`1px solid ${T.sidebarA}` }}>
          <div style={{ ...S.row, gap:10, marginBottom:10 }}>
            <div style={{ width:32, height:32, borderRadius:"50%", background:T.accent, display:"flex", alignItems:"center", justifyContent:"center", color:"#fff", fontWeight:700, fontSize:13 }}>L</div>
            <div>
              <div style={{ color:"#FFFFFF", fontSize:13, fontWeight:600 }}>Leandro</div>
              <div style={{ color:"#8FAB96", fontSize:11 }}>Store Manager</div>
            </div>
          </div>
          <button onClick={() => setShowLogin(true)} style={{ ...S.btn("ghost"), fontSize:12, color:"#8FAB96", padding:"6px 0", width:"100%" }}>
            ← View Login Screen
          </button>
        </div>
      </div>
