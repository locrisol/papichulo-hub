...
**Hiding what a role cannot use.** The sidebar shows every item to every role, and opening a page you do not have access to gives an empty screen rather than a refusal. The data is safe either way, since the database enforces the rules, but the app should be honest about it.
